...
if (isset($_POST['apply'])){ /*Because the form first click is the button, we set by using the Name given to the button & we used $_POST because the
     ...
     if (empty($FirstName)){
       ...
       if (count($error)==0){
                  $query = "INSERT INTO staff(FirstName,LastName,Email,Phone,Street,City,BirthDate,Departement,Gender) 
                  VALUES('$FirstName','$LastName', '$Email','$Phone','$Street','$City','$BirthDate','$Departement','$Gender') ";
                  $result = mysqli_query($con, $query);
                 $query1 = "SELECT ID from staff where Email='$Email' ";
                 $res2= mysqli_query($con, $query1);
                 $row = mysqli_fetch_array($res2);
                  $ide = $row['ID'];
                $query2 = "UPDATE  doctors set EducationDegree ='$BA' where Doctor_ID='$ide'";
                 ...
                 <div class="row"><!--After setting Page seen Above, You need to set ROW to allow you divide the 12px into sections-->
                       ...
                       <div class="col-md-6 jumbotron"> <!--2nd section if (6px) which will contain the Application Form for DOCTORS-->
                           ...
                           <form method="post" action=""> <!--Form tag using HTTP Request(POST) which will POST Data into the database Table siting on the serve  -->
                               ...
                               <div class="form-group">
                                   <label for="">EducationDegree</label>
                                   <input type="text" name="BA" class="form-control" autocomplete="off" placeholder="Enter your EducationDegree" value="<?php if (isset($_POST['BA'])) echo $_POST['BA']; ?>">
                               </div> 
